...
A [cache in Github Actions](https://github.com/actions/cache) stores files so they don't have to be re-uploaded each time an action runs. This is very useful for saving time.

## How to set up and maintain the cache

The cache can be set up and maintained with a single workflow. The workflow should have both a `workflow_dispatch` trigger (so you can run it to create the cache) and a `schedule` trigger (so it runs periodically for maintenance). The cache must be maintained because if more than 7 days pass without a cache hit, Github will delete the cache, and it must be installed again the next time your action runs. By running a cache hit on a schedule, your cache will not be destroyed even if you go more than one week between actively using the cache.
...
Load the R packages. If there was a cache hit (as in a "maintenance" run), this step is skipped. If there was no cache hit (as when setting up the cache), this stepp executes and installs your R packages. The amount of time that this step takes depends on how many packages you need.
```yaml        
...
Once you have created a cache in a repo, you can reference it in other Github Actions workflows by including the "Check cached R packages" step in your YAML.
