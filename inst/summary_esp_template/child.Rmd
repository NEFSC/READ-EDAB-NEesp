### `r eval(input$si_file$name)`
```{r}
dat <- NEesp::prep_si_data(
  file_path = eval(input$si_file$datapath),
  metric = eval(input$si_metric),
  var = eval(input$var_options)
)

plt <- NEesp::plot_corr_only(
  data = dat,
  lag = as.numeric(input$si_lag),
  species = eval(input$si_species),
  mode = "download"
)
print(plt)

rpt_card_ind <- rbind(rpt_card_ind, NEesp::ind_rpt(
  data = dat,
  var = eval(input$var_options),
  metric = eval(input$si_metric)
))

rpt_card_time <- dplyr::full_join(rpt_card_time,
  NEesp::time_rpt(data = dat, 
           out_name = eval(input$var_options)),
  by = "Time"
)
```
