...
ggplot2::ggplot(
  data,
  ggplot2::aes(
    y = reorder(new_label, -total_risk),
    x = Indicator %>%
      stringr::str_replace_all("_", " "),
    fill = norm_rank
  )
...
dat <- guild_data %>%
  ...
  dplyr::select(
    Guild, n_guild_species, category, Indicator, avg_guild_risk,
    total_guild_risk
  ) %>%
  dplyr::mutate(
    avg_guild_risk = avg_guild_risk %>%
      round(digits = 2) %>%
      format(
        scientific = FALSE,
        big.mark = ","
