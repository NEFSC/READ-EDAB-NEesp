...
plot_asmt <- function(x, metric, ytitle = "", lin = lines, col = colors){
  ...
  if(nrow(x) > 0){
    ...
    fig <- ggplot2::ggplot(x,
      ...
      ggplot2::guides(shape = guide_legend(ncol = 2,
    ...
    if(metric != "Catch" &
       x$Category %>% unique() %>% length() > 2) {
      fig <- fig + 
        ggplot2::facet_grid(rows = vars(Category),
                   scales = "free_y")
    }
...
status <- function(data, regions, metric){
  ...
  if(sum(is.na(data)) == length(data)){
    ...
    if(data_info == "PRESENT"){
      if(metric == "bbmsy"){
        data_status <- if(tail(data, n = 1) > 1) {"GOOD"} else 
          if(tail(data, n = 1) > 0.5) {"CAUTION"} else {"DANGER"}
      }
      if(metric == "ffmsy"){
