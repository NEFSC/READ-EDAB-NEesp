...
get_diet_plot <- function(data){
  ...
  if(nrow(data) > 0){
    ...
    if(length(normalized$n_predators) > 1){
      ...
      # plot
      fig <- ggplot2::ggplot(new_normalized,
                             ggplot2::aes(x = year,
                        y = prop2,
                        fill = gensci))+
        ggplot2::geom_bar(color = "black", stat = "identity")+
        ggplot2::scale_fill_manual(name = "Prey \ncategory",
                          values = plot_colors)+
...
plot_swept <- function(x, var){
  ...
  if(nrow(x) > 0){
    fig <- ggplot2::ggplot(x)+
      ggplot2::geom_ribbon(ggplot2::aes(x = YEAR,
                                        ymin = value - 2*error,
                                        ymax = value + 2*error,
                                        fill = Season),
                           alpha = 0.5)+
    ...
    if (nrow(y) > 0) { 
      fig <- fig + 
        ecodata::geom_gls(ggplot2::aes(x = YEAR,
                                       y = value,
                                       color = Season)) 
    }
...
plot_climate_vulnerability <- function(data){
  ...
  for(i in unique(data$Attribute.Category)){
    ...
    fig <- ggplot2::ggplot(plot_data,
                           ggplot2::aes(x = name,
                      y = value,
                      fill = name))+
      ggplot2::geom_bar(stat = "identity",
               color = "black")+
      ggplot2::facet_wrap(facets = ggplot2::vars(Attribute),
                 ncol = 2)+
