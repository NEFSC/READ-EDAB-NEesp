...
for (i in 1:nrow(rpt_card_time)) {
  for (j in 1:ncol(rpt_card_time)) {
    ...
    # make matrix for color setting later
    ...
    # remove words
    rpt_card_time[i, j] <- stringr::str_remove_all(rpt_card_time[i, j], pattern = ", high")
    rpt_card_time[i, j] <- stringr::str_remove_all(rpt_card_time[i, j], pattern = ", neutral")
    rpt_card_time[i, j] <- stringr::str_remove_all(rpt_card_time[i, j], pattern = ", low")
  }
}
...
names <- colnames(rpt_card_time)
names <- sapply(names, stringr::str_wrap, width = 5)
names <- sapply(names, stringr::str_replace_all, pattern = "_", replacement = "\n")

last_row <- nrow(rpt_card_time)
rpt_card_time[c(last_row - 1, last_row), ] <- sapply(rpt_card_time[c(last_row - 1, last_row), ],
  stringr::str_wrap,
  width = 5
)

# make table
tab <- ggpubr::ggtexttable(rpt_card_time,
  rows = NULL,
  cols = names,
...
for (i in 1:nrow(index2)
  ...
  # CANNOT have row = 1
  if (r != 1) {
    tab <- ggpubr::table_cell_bg(tab,
      row = r, column = c, linewidth = 5,
...
```{r, rpt-card-flex}
# format colnames
colnames(rpt_card_time) <- sapply(colnames(rpt_card_time), 
                                  stringr::str_replace_all, 
                                  pattern = "_", 
                                  replacement = "\n")

ft <- flextable::flextable(rpt_card_time)

for (i in 1:nrow(rpt_card_time)) {
  for (j in 1:ncol(rpt_card_time)) {
    
    # color based on ecosystem favorability
    if(stringr::str_detect(toString(rpt_card_time[i, j]), ", high")){
